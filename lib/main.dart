...
class AurenzaApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        home: backendReady ? const BrokerShell() : const BackendUnavailableScreen(),
      );
}
...
class BackendUnavailableScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
        body: SafeArea(
          child: Center(
            child: AppError(
              message: 'AURENZA could not connect to its backend.\n\nConfigure Supabase and restart the application.',
              onRetry: main,
            ),
          ),
        ),
              ...
              return Row(
                children: [
                  _DesktopSidebar(
                    ...
                    onAdmin: AurenzaAdminAccess.isAdmin ? openAdmin : null,
                  ),
                  Expanded(child: _Page(title: titles[selectedIndex], index: selectedIndex)),
                ],
              );
            }
          ...
          child: Column(
            children: [
              const Padding(
                ...
                child: Row(
                  children: [
                    Icon(Icons.diamond_outlined, color: AurenzaColors.gold, size: 28),
                    SizedBox(width: 10),
                    Text('AURENZA', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 20, letterSpacing: 1.2)),
                  ],
                ),
              ),
              Expanded(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    return Padding(
                      ...
                      child: ListTile(
                        ...
                        leading: Icon(icons[index], color: selected ? AurenzaColors.gold : Colors.white70),
                        title: Text(titles[index], style: TextStyle(color: selected ? Colors.white : Colors.white70, fontWeight: selected ? FontWeight.w700 : FontWeight.w500)),
              ...
              if (onAdmin != null)
                Padding(
                  ...
                  child: ListTile(
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                    leading: const Icon(Icons.admin_panel_settings_outlined, color: AurenzaColors.gold),
                    title: const Text('Admin Console', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                    subtitle: const Text('Authorized', style: TextStyle(color: Colors.white54, fontSize: 11)),
                    onTap: onAdmin,
                  ),
                ),
              const Padding(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('SANDBOX MODE', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w700, fontSize: 11)),
                  ],
                ),
              ),
            ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (onAdmin != null) IconButton(onPressed: onAdmin, tooltip: 'Admin Console', icon: const Icon(Icons.admin_panel_settings_outlined)),
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.show_chart_outlined), label: 'Markets'),
...
class _Page extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (index == 0) {
      return Column(children: [if (MediaQuery.sizeOf(context).width >= 700) _Header(title: title), const Expanded(child: DashboardScreen())]);
    }
    if (index == 5) {
      return Column(children: [if (MediaQuery.sizeOf(context).width >= 700) _Header(title: title), const Expanded(child: InvestmentProfitScreen())]);
    }
    return Column(
      children: [
        ...
        Expanded(
          child: Center(
            child: Column(
              ...
              children: [
                const Icon(Icons.construction_outlined, size: 48, color: AurenzaColors.gold),
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(
          children: [
            ...
            const CircleAvatar(backgroundColor: AurenzaColors.forest, child: Icon(Icons.person_outline, color: Colors.white)),
          ],
        ),
      );
}
