    ...
    return Scaffold(
      body: LayoutBuilder(builder: (context, constraints) {
        if (constraints.maxWidth >= 1100) {
          return Row(children: [
            _AdminDesktopSidebar(selectedIndex: selectedIndex, onSelected: (index) => setState(() => selectedIndex = index)),
            Expanded(child: _AdminPage(sectionIndex: selectedIndex, section: sections[selectedIndex])),
          ]);
        }
        if (constraints.maxWidth >= 700) {
          return Row(children: [
            NavigationRail(
              ...
              destinations: [for (var i = 0; i < icons.length; i++) NavigationRailDestination(icon: Icon(icons[i]), selectedIcon: Icon(icons[i]), label: Text(sections[i]))],
            ),
            Expanded(child: _AdminPage(sectionIndex: selectedIndex, section: sections[selectedIndex])),
          ]);
        }
        return Scaffold(
          ...
          body: _AdminPage(sectionIndex: selectedIndex, section: sections[selectedIndex]),
...
class _AdminDesktopSidebar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: SafeArea(child: Column(children: [
          ...
          const Padding(padding: EdgeInsets.all(18), child: Row(children: [
...
class _AdminPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Expanded(child: finance ? const AdminFinanceScreen() : overview ? const _AdminOverview() : _AdminPlaceholder(section: section)),
    ]);
  }
}
...
class _AdminOverview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const metrics = <(String, String, IconData)>[
      ('Users', '—', Icons.people_outline), ('KYC Verified', '—', Icons.verified_user_outlined),
      ('Active Traders', '—', Icons.show_chart_outlined), ('Company Wallet', 'Separate', Icons.account_balance_outlined),
      ('User Wallets', 'Per customer', Icons.account_balance_wallet_outlined), ('Open Trades', '—', Icons.swap_vert_rounded),
      ('Today P&L', '—', Icons.trending_up), ('Broker Status', 'Checking', Icons.link_outlined),
    ];
    return ListView(padding: const EdgeInsets.all(28), children: [
      ...
      const Text('Company treasury is separated from every customer wallet. Finance monitoring is tied to the exact authenticated customer wallet.'),
      const SizedBox(height: 24),
      GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: 280, mainAxisExtent: 130, crossAxisSpacing: 16, mainAxisSpacing: 16), itemCount: metrics.length, itemBuilder: (context, index) {
        ...
        return Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Icon(metric.$3, color: AurenzaColors.gold), const Spacer(), Text(metric.$1), const SizedBox(height: 4), Text(metric.$2, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
        ])));
      }),
      const SizedBox(height: 24),
      const Card(child: ListTile(leading: Icon(Icons.info_outline, color: AurenzaColors.gold), title: Text('Finance safety boundary'), subtitle: Text('The admin Finance screen never treats a company balance as a customer balance. Withdrawal workflows must resolve the requesting user ID to that exact user wallet before approval.'))),
    ]);
  }
}
...
class _AdminPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    const Icon(Icons.construction_outlined, size: 54, color: AurenzaColors.gold), const SizedBox(height: 16), Text(section, style: Theme.of(context).textTheme.headlineSmall), const SizedBox(height: 8), const Text('Admin module registered. Screen, repository, permissions, backend RPCs, and tests will be implemented in its module phase.'),
  ]));
}
...
class _AdminDeniedScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('AURENZA Admin')), body: const Center(child: Padding(padding: EdgeInsets.all(32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    Icon(Icons.lock_outline, size: 56, color: AurenzaColors.gold), SizedBox(height: 18), Text('Admin access required', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w800)), SizedBox(height: 10), Text('Your authenticated account does not have an authorized AURENZA admin role.'),
  ]))));
}
